...
## Download Database
You can download pre-built databases [here](https://jaebeom-kim.github.io/metabuli-doc/databases/new-database/).
...
- `--custom-metamer`: Custom metamer JSON file. See [Custom Metamer Example](../modules/build.md/#custom-metamer-example) 
- `--syncmer`: Set `1` to use syncmers 
...
- `--cds-info`: File containing CDS file paths. Provided CDSs are used instead of Prodigal's gene prediction. Only GenBank/RefSeq CDS files are supported.
